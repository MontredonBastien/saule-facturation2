...
-- Fonction pour mettre à jour la date du dernier accès
CREATE OR REPLACE FUNCTION update_accountant_last_access()
RETURNS trigger AS $$
BEGIN
  UPDATE accountant_company_access
  SET last_access = now()
...
-- Insérer les données de test pour démonstration
DO $$
DECLARE
  accountant1_id uuid;
  accountant2_id uuid;
  ...
  company2_id uuid;
BEGIN
  -- Vérifier si des comptables de test existent déjà
  IF NOT EXISTS (SELECT 1 FROM accountants WHERE email = 'comptable1@cabinet.fr') THEN
    -- Créer des comptables de test
    INSERT INTO accountants (email, full_name, company_name, phone, is_active)
    VALUES
      ('comptable1@cabinet.fr', 'Marie Dubois', 'Cabinet Dubois Expertise', '01 23 45 67 89', true),
      ('comptable2@cabinet.fr', 'Pierre Martin', 'Martin Comptabilité', '01 98 76 54 32', true)
    RETURNING id INTO accountant1_id;

    -- Récupérer les IDs des premières entreprises (si elles existent)
    SELECT id INTO company1_id FROM companies LIMIT 1;
    SELECT id INTO company2_id FROM companies LIMIT 1 OFFSET 1;

    -- Créer des accès de test si les entreprises existent
    IF company1_id IS NOT NULL THEN
      INSERT INTO accountant_company_access (accountant_id, company_id, access_level, is_active)
      VALUES
        (accountant1_id, company1_id, 'full_admin', true);
    END IF;
  END IF;
END $$;
